...
from pddl_planning.iiwa_utils import DOOR_CLOSED, DOOR_OPEN, open_wsg50_gripper
from pddl_planning.utils import get_model_name, create_transform, get_movable_joints, get_body_boxes, get_base_body, \
    ...
    get_aabb_z_placement, get_body_pose, weld_to_world, get_world_pose
...
def load_station(time_step=0.0, **kwargs):
    brick_file_path = FindResourceOrThrow(
        "drake/examples/manipulation_station/models/061_foam_brick.sdf")
    # Slightly off IK solution will cause the grasp & placement to be off
    ...
    model_name = 'soup'
    item_file_path = SDF_PATH_FROM_NAME[model_name] # brick_file_path
    item = AddModelFromSdfFile(file_name=item_file_path, model_name=model_name,
    ...
    #end_z = get_z_placement(plant, box_from_geom, item, shelf_body, shelf_index)
    # TODO: can also place on the other shelf
    ...
    print(start_x, start_y, start_z, start_theta)

    initial_poses = {
        item: create_transform(translation=[start_x, start_y, start_z], rotation=[0, 0, start_theta]),
    }
    #goal_poses = {
    #    item: create_transform(translation=[end_x, end_y, end_z], rotation=[0, 0, end_theta]),
    #}
...
def load_dope(time_step=0.0, dope_path=DOPE_PATH, goal_name='soup', **kwargs):
    station = ManipulationStation(time_step, IiwaCollisionModel.kBoxCollision)
    plant = station.get_mutable_multibody_plant()
    scene_graph = station.get_mutable_scene_graph()
    station.AddCupboard()
    ...
    for name in pose_from_name:
        model_from_name[name] = AddModelFromSdfFile(file_name=SDF_PATH_FROM_NAME[name], model_name=name,
                                                    plant=plant, scene_graph=scene_graph)
    station.Finalize()
