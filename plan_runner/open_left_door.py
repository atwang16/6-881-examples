...
def GenerateOpenLeftDoorPlansByImpedanceOrPosition(
    ...
    :param handle_angle_start the initial door angle, from 0 to np.pi/2
    """
    def ReturnConstantOrientation(i, num_knot_points):
        return R_WEa_ref
    print "handle angle start:", handle_angle_start
    print "handle angle end:  ", handle_angle_end
    if handle_angle_start < handle_angle_end:
        ...
        if open_door_method == "Impedance":
            ...
            plan_list.append(OpenLeftDoorImpedancePlan(
                angle_start=handle_angle_start,
                angle_end=handle_angle_end,
                duration=open_door_duration))
        ...
        # TODO: add support for opening door fully afterward
